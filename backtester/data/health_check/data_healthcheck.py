...
class StreamlinedHealthChecker:
    ...
    def __init__(
        ...
        interpolation_strategy: str = "financial_forward_fill",
    ):
    ...
    def add_issue(
        self, severity: str, category: str, message: str, fix_commands: List[str] = None
    ):
    ...
    def check_data_gaps(
        self, data: vbt.Data, exchange: str, market: str, timeframe: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(close_df.index) > 0:
                ...
                if total_days > 365:  # More than 1 year of data
                    ...
                    critical_threshold = max(
                        5000, total_days * 2
                    )  # ~2 periods per day as critical
                    minor_threshold = max(
                        1000, total_days * 0.5
                    )  # ~0.5 periods per day as minor
                else:
            ...
            if gap_info["critical_gaps"]:
                critical_symbols = [
                    g["symbol"] for g in gap_info["critical_gaps"][:5]
                ]  # Limit to 5 symbols
                ...
                avg_missing = gap_info["total_missing"] // len(
                    gap_info["critical_gaps"]
                )
                completion_rates = [
                    g["completion_rate"] for g in gap_info["critical_gaps"]
                ]
                ...
                if self.auto_fix:
                    self.apply_fix(
                        f"Fill critical gaps for {len(critical_symbols)} symbols",
                        fix_commands,
                    )

            elif gap_info["minor_gaps"]:
                avg_missing = (
                    gap_info["total_missing"] // len(gap_info["minor_gaps"])
                    if gap_info["minor_gaps"]
                    else 0
                )
                ...
                if (
                    ...
                    and self.auto_fix
                    and gap_info["total_missing"] <= 10000
                ):
                    self.add_issue(
                        "warning",
                        "data_gaps",
                    ...
                    if self.apply_interpolation_fix(
                        data, exchange, market, timeframe, {}, gap_info
                    ):
                        print("   🔧 Applied interpolation fix for minor gaps")
                        self.apply_fix(
                            f"Interpolate {gap_info['total_missing']} missing periods",
            ...
            self.add_issue(
                "critical",
                "analysis_error",
                f"Gap analysis failed for {exchange}/{market}/{timeframe}: {e}",
            )
        ...
        try:
            ...
            for exchange in cache_stats["exchanges"]:
                ...
                if not self.cache_manager.is_volume_cache_fresh(exchange):
                    fix_commands = [
                        f"python backtester/scripts/fetch_data_cli.py --exchange {exchange} --top 20"
                    ]
                    self.add_issue(
                        "warning",
                        "cache_stale",
                    ...
                    if self.auto_fix:
                        self.apply_fix(
                            f"Refresh volume cache for {exchange}", fix_commands
                        )
            ...
            self.add_issue(
                "critical", "cache_error", f"Cache integrity check failed: {e}"
            )
        ...
        try:
            ...
            if data is None:
                self.add_issue(
                    "critical",
                    "load_error",
                    f"Cannot load {exchange}/{market}/{timeframe}",
                )
                return analysis

            analysis["has_data"] = True
            analysis["symbol_count"] = (
                len(data.symbols) if hasattr(data, "symbols") else 0
            )
            ...
            if fixes_applied_count > 0 and self.auto_fix:
                ...
                data_reloaded = data_storage.load_data(
                    exchange, timeframe, market_type=market
                )
                if data_reloaded is not None:
                    # Re-analyze gaps with fresh data
                    analysis["gaps"] = self.check_data_gaps(
                        data_reloaded, exchange, market, timeframe
                    )
                    data = data_reloaded  # Use reloaded data for freshness check
                    analysis["fixes_applied"] = self.fixes_applied[
                        -fixes_applied_count:
                    ]
                    print(f"   ♻️  Data reloaded and re-analyzed")

            # Check freshness (using potentially reloaded data)
            analysis["freshness"] = self.check_data_freshness(
                data, exchange, market, timeframe
            )

            missing_count = analysis["gaps"]["total_missing"]
            print(
                f"   ✅ {analysis['symbol_count']} symbols, {missing_count} missing periods"
            )
            ...
            self.add_issue(
                "critical",
                "analysis_error",
                f"Analysis failed for {exchange}/{market}/{timeframe}: {e}",
            )
    ...
    def generate_report(self, analyses: List[Dict[str, Any]]) -> str:
        ...
        if self.fixes_applied:
            ...
            for fix in self.fixes_applied:
                ...
                lines.append(
                    f"   Commands: {fix['success_count']}/{fix['total_commands']} successful"
                )
            lines.append("")
        ...
        if analyses:
            ...
            problem_files = [
                a
                for a in analyses
                if a["gaps"]["critical_gaps"] or a["freshness"].get("is_stale", False)
            ]
            if problem_files:
                ...
                for analysis in problem_files[:5]:
                    ...
                    lines.append(
                        f"📄 {info['exchange']}/{info['market']}/{info['timeframe']}"
                    )
                    if analysis["gaps"]["critical_gaps"]:
                        lines.append(
                            f"   Gaps: {len(analysis['gaps']['critical_gaps'])} symbols"
                        )
                    if analysis["freshness"].get("is_stale", False):
                        lines.append(
                            f"   Stale: {analysis['freshness']['age_hours']:.1f}h old"
                        )
                lines.append("")
        ...
        if len(self.issues["critical"]) > 0:
            ...
            lines.append(
                "  1. Run with --auto-fix to resolve critical issues automatically"
            )
            ...
            has_minor_gaps = any(
                "minor gaps" in issue["message"] for issue in self.issues["warning"]
            )
            if has_minor_gaps:
        ...
        lines.append(
            "• Note: Thresholds scale with data timespan for realistic assessment"
        )
        lines.append("")
        lines.append("📊 INTERPOLATION STRATEGIES")
        lines.append("-" * 40)
        lines.append(
            "• financial_forward_fill: OHLC=last_close, Volume=0 (recommended)"
        )
    ...
    def interpolate_missing_data(
        ...
        market: str,
        timeframe: str,
        strategy: str = "financial_forward_fill",
    ) -> Optional[vbt.Data]:
        """
        ...
        try:
            ...
            complete_index = pd.date_range(
                start=original_index[0], end=original_index[-1], freq=pandas_freq
            )
            ...
            print(
                f"   📊 Original: {original_len} periods, Complete: {complete_len} periods (+{missing_count} interpolated)"
            )
            ...
            for component in ["open", "high", "low", "close", "volume"]:
                ...
                if strategy == "financial_forward_fill":
                    ...
                    interpolated_ohlcv[component] = reindexed.interpolate(
                        method="linear"
                    )
            ...
            if strategy == "financial_forward_fill":
                ...
                for symbol in symbols:
                    ...
                    original_mask = interpolated_ohlcv["close"].index.isin(
                        original_index
                    )
                    interpolated_mask = ~original_mask

                    if (
                        interpolated_mask.any()
                        and symbol in interpolated_ohlcv["close"].columns
                    ):
                        ...
                        for component in ["open", "high", "low"]:
                            if (
                                component in interpolated_ohlcv
                                and symbol in interpolated_ohlcv[component].columns
                            ):
                                interpolated_ohlcv[component].loc[
                                    interpolated_mask, symbol
                                ] = prev_close[interpolated_mask]
            ...
            try:
                ...
                for symbol in symbols:
                    ...
                    for component in ["open", "high", "low", "close", "volume"]:
                        if (
                            ...
                            symbol_data[component.capitalize()] = interpolated_ohlcv[
                                component
                            ][symbol]
                ...
                if new_data is None:
                    print(
                        f"   ❌ Failed to create VBT Data object from interpolated data"
                    )
                    return None
    ...
    def apply_interpolation_fix(
        ...
        try:
            ...
            if total_missing > 10000:  # More than ~7 days of 1m data
                print(
                    f"   ⚠️  Skipping interpolation: {total_missing} missing periods too large"
                )
                return False
...
Examples:
  ...
  %(prog)s --interpolate --interpolation-strategy linear  # Use linear interpolation
    ...
    parser.add_argument("--exchange", type=str, help="Check specific exchange only")
    parser.add_argument("--timeframe", type=str, help="Check specific timeframe only")
    parser.add_argument(
        "--market",
        type=str,
        choices=["spot", "swap"],
    ...
    parser.add_argument(
        "--auto-fix", action="store_true", help="Automatically fix critical issues"
    )
    parser.add_argument(
        "--interpolate",
        action="store_true",
    ...
    if args.auto_fix:
        if args.interpolate:
            print(
                f"🔧 Auto-fix enabled with interpolation ({args.interpolation_strategy})"
            )
        else:
            print("🔧 Auto-fix enabled - will resolve critical issues")
    elif args.interpolate:
        print(
            f"📊 Interpolation analysis enabled ({args.interpolation_strategy}) - use --auto-fix to apply"
        )
