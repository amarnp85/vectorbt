...
class SimpleDataFetcher:
    ...
    def _extract_volume_from_data(
        self, data: Dict[str, Any], exchange_id: str
    ) -> Optional[float]:
    ...
    def get_market_data(
        ...
        if (
            ...
            and self.cache.is_volume_cache_fresh(exchange_id)
        ):
            ...
            if cached_volumes:
                ...
                try:
                    ...
                    logger.info(
                        f"Retrieved {len(result)} symbols from cache in {duration:.2f} seconds"
                    )
                    return result

                except Exception as e:
                    logger.warning(
                        f"Error using cached data, falling back to fresh fetch: {e}"
                    )
        ...
        try:
            ...
            symbols_needing_volume = [
                s for s, data in result.items() if "volume" not in data
            ]

            if symbols_needing_volume and hasattr(exchange, "fetch_tickers"):
                logger.info(
                    f"Fetching volume for {len(symbols_needing_volume)} symbols via tickers"
                )
                ...
                for symbol in symbols_needing_volume:
                    if symbol in result:
                        ...
                        if exchange_id.lower() == "binance" and market_type in [
                            "swap",
                            "future",
                        ]:
                            ...
                            if symbol in markets:
                                ...
                                if (
                                    "linear" in market_info
                                    or market_info.get("linear") is True
                                ):
                                    subtype = "linear"
                                elif (
                                    "inverse" in market_info
                                    or market_info.get("inverse") is True
                                ):
                                    subtype = "inverse"
                                elif "info" in market_info and isinstance(
                                    market_info["info"], dict
                                ):
                                    info = market_info["info"]
                                    if (
                                        info.get("contractType") == "INVERSE"
                                        or info.get("subType") == "inverse"
                                    ):
                                        subtype = "inverse"

                        # Create a compound key for grouping
                        group_key = (
                            f"{market_type}_{subtype}"
                            if market_type in ["swap", "future"]
                            and exchange_id.lower() == "binance"
                            else market_type
                        )
                ...
                logger.info(
                    f"Grouped symbols by type: {[(t, len(symbols)) for t, symbols in symbols_by_type.items()]}"
                )
                ...
                for market_type, symbols_of_type in symbols_by_type.items():
                    ...
                    logger.info(
                        f"Fetching {len(symbols_of_type)} {market_type} symbols"
                    )
                    ...
                    for i in range(0, len(symbols_of_type), batch_size):
                        ...
                        try:
                            ...
                            for symbol, ticker in tickers.items():
                                volume = self._extract_volume_from_data(
                                    ticker, exchange_id
                                )
                            ...
                            logger.debug(
                                f"Successfully fetched {len(tickers)} {market_type} tickers"
                            )
                            ...
                            logger.warning(
                                f"Error fetching {market_type} tickers batch: {e}"
                            )
                            ...
                            for symbol in batch:
                                try:
                                    ...
                                    logger.debug(
                                        f"Error fetching ticker for {symbol}: {individual_e}"
                                    )
                                    continue
            ...
            logger.info(
                f"Fetched {len(result)} symbols ({symbols_with_volume} with volume) in {duration:.2f} seconds"
            )
    ...
    def get_inception_timestamps(
        ...
        if use_cache:
            ...
            if len(symbols_to_fetch) < len(symbols):
                logger.info(
                    f"Using {len(symbols) - len(symbols_to_fetch)} cached timestamps"
                )
        ...
        logger.info(
            f"Fetching inception timestamps for {len(symbols_to_fetch)} symbols"
        )
        ...
        if failed_symbols:
            logger.warning(
                f"Failed to get timestamps for {len(failed_symbols)} symbols"
            )
    ...
    def get_market_data_with_timestamps(
        self,
        exchange_id: str,
        quote_currency: Optional[str] = None,
        market_types: Optional[List[str]] = None,
