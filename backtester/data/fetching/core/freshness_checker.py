...
class FreshnessChecker:
    ...
    @staticmethod
    def get_expected_latest_candle(
        current_time: pd.Timestamp, timeframe: str
    ) -> pd.Timestamp:
        """
        ...
        if tf.endswith("d") or "day" in tf:
            ...
            days = (
                int(tf[:-3] if tf.endswith("day") else tf[:-1])
                if tf[:-1].isdigit()
                else 1
            )
            ...
            hours = (
                int(tf[:-4] if tf.endswith("hour") else tf[:-1])
                if tf[:-1].isdigit()
                else 1
            )
            ...
            if latest_complete_hour < 0:
                ...
                return (current_time - timedelta(days=1)).replace(
                    hour=24 + latest_complete_hour, minute=0, second=0, microsecond=0
                )
            else:
                return current_time.replace(
                    hour=latest_complete_hour, minute=0, second=0, microsecond=0
                )

        elif tf.endswith("m") or "min" in tf:
            # Minute candles complete at the top of the next minute
            minutes = (
                int(tf[:-3] if tf.endswith("min") else tf[:-1])
                if tf[:-1].isdigit()
                else 1
            )
            ...
            if latest_complete_minutes < 0:
                ...
                return (current_time - timedelta(days=1)).replace(
                    hour=23,
                    minute=60 + latest_complete_minutes,
                    second=0,
                    microsecond=0,
                )
            else:
    ...
    @staticmethod
    def get_safe_end_time_for_fetch(
        current_time: pd.Timestamp, timeframe: str
    ) -> pd.Timestamp:
        """
        ...
        """
        latest_complete_start = FreshnessChecker.get_expected_latest_candle(
    ...
    @staticmethod
    def is_data_fresh(
        data: vbt.Data, target_end: Optional[Union[str, pd.Timestamp]], timeframe: str
    ) -> bool:
        """
        ...
        try:
            ...
            expected_latest = FreshnessChecker.get_expected_latest_candle(
                target_dt, timeframe
            )
            ...
            if hasattr(data, "symbols") and len(data.symbols) > 1:
                ...
                return FreshnessChecker._is_symbol_fresh(
                    data, None, expected_latest, timeframe
                )
    ...
    def identify_stale_symbols(
        ...
        try:
            ...
            for symbol in symbols:
                if FreshnessChecker._is_symbol_fresh(
                    data, symbol, expected_latest, timeframe
                ):
