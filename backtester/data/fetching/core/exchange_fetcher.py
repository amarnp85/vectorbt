...
class ExchangeFetcher:
    ...
    def fetch_symbols(
        ...
        try:
            ...
            if len(symbols) <= 10:
                ...
                logger.info(
                    f"Symbols: {', '.join(symbols[:5])}... (+{len(symbols)-5} more)"
                )

            # Prepare fetch kwargs
            fetch_kwargs = {"show_progress": False}  # We'll handle progress ourselves
            ...
            if data is not None:
                ...
                successful_symbols = (
                    list(data.symbols) if hasattr(data, "symbols") else []
                )
                ...
                if failed_symbols:
                    logger.warning(
                        f"Failed to fetch {len(failed_symbols)} symbols: {list(failed_symbols)}"
                    )
    ...
    def _log_symbol_results(
        ...
        try:
            ...
            fetch_logger.log_symbol_progress(
                len(successful_symbols), len(requested_symbols)
            )
    ...
    def _get_common_start_date(
        self, symbols: List[str], start_dates: Dict[str, str]
    ) -> Optional[str]:
    ...
    def _get_safe_end_date(
        self, end_date: Optional[Union[str, vbt.timestamp]], timeframe: str
        ...
        try:
            ...
            if end_date is None or (
                isinstance(end_date, str) and end_date.lower() == "now"
            ):
                ...
                safe_end_pd = FreshnessChecker.get_safe_end_time_for_fetch(
                    current_pd, timeframe
                )
