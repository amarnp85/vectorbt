...
class FetchLogger:
    ...
    def log_start(self, start_date: Optional[str], end_date: Optional[str]):
        ...
        if len(self.requested_symbols) <= 10:
            ...
            logger.debug(f"[{utc_time}] Requested {len(self.requested_symbols)} symbols: {self.requested_symbols[:5]}...")
    ...
    def log_filter_result(self, filtered_symbols: List[str]):
        ...
        for symbol in filtered_symbols:
            self.symbol_progress[symbol] = {'status': 'pending', 'source': None, 'time': None}
    ...
    def log_cache_result(self, status: str, details: Dict[str, Any]):
        ...
        if status == "hit":
            cache_table.add_row("✅ Found", str(len(self.filtered_symbols)), 
                              ", ".join(self.filtered_symbols[:3]) + ("..." if len(self.filtered_symbols) > 3 else ""),
                              "Use cached data")
            # Mark all symbols as cached
            for symbol in self.filtered_symbols:
                self.symbol_progress[symbol] = {'status': 'cached', 'source': 'cache', 'time': 0}
                
        elif status == "partial":
            available = details.get('available', 0)
            missing = details.get('missing', 0)
            stale = details.get('stale', 0)
            ...
            if cached_symbols:
                cache_table.add_row("✅ Fresh", str(len(cached_symbols)), 
                                  ", ".join(list(cached_symbols)[:3]) + ("..." if len(cached_symbols) > 3 else ""),
                                  "Use from cache")
            ...
            if stale_symbols:
                cache_table.add_row("🔄 Stale", str(len(stale_symbols)), 
                                  ", ".join(list(stale_symbols)[:3]) + ("..." if len(stale_symbols) > 3 else ""),
                                  "Update recent data")
            
            if missing_symbols:
                cache_table.add_row("❌ Missing", str(len(missing_symbols)), 
                                  ", ".join(list(missing_symbols)[:3]) + ("..." if len(missing_symbols) > 3 else ""),
                                  "Fetch from inception")
            
        else:
            cache_table.add_row("❌ Miss", "0", "No cached data found", "Fetch all from exchange")
    ...
    def log_resample_result(self, success: bool, source_tf: Optional[str] = None):
        ...
        resample_table = Table(title=f"🔄 Resample Analysis ({utc_time})", box=box.ROUNDED)
        ...
        if success:
            resample_table.add_row("✅ Success", f"Resampled from {source_tf}" if source_tf else "Resampled data available", "Use resampled data")
            logger.info(f"[{utc_time}] Successfully resampled from {source_tf} timeframe")
            
            # Mark symbols as resampled
            for symbol in self.filtered_symbols:
                if self.symbol_progress[symbol]['status'] == 'pending':
                    self.symbol_progress[symbol] = {'status': 'resampled', 'source': f'resample_{source_tf}', 'time': 0}
        else:
            resample_table.add_row("⏭️ Skip", "No suitable lower timeframe data", "Continue to exchange")
            logger.debug(f"[{utc_time}] No suitable lower timeframe data for resampling")
    ...
    def log_exchange_start(self, symbols: List[str]):
        ...
        exchange_table = Table(title=f"🔄 Exchange Fetch Starting ({utc_time})", box=box.ROUNDED)
        exchange_table.add_column("Metric", style="cyan")
        exchange_table.add_column("Value")
        exchange_table.add_column("Details", style="dim")
        
        exchange_table.add_row("Symbols", str(len(symbols)), ", ".join(symbols[:5]) + ("..." if len(symbols) > 5 else ""))
        ...
        logger.info(f"[{utc_time}] Starting exchange fetch for {len(symbols)} symbols: {symbols[:5]}{'...' if len(symbols) > 5 else ''}")
    ...
    def log_exchange_result(self, success: bool, symbol_count: Optional[int] = None, 
        ...
        exchange_table = Table(title=f"📊 Exchange Results ({utc_time})", box=box.ROUNDED)
        exchange_table.add_column("Metric", style="cyan")
        exchange_table.add_column("Value", justify="right")
        exchange_table.add_column("Details")
        
        if success:
            exchange_table.add_row("✅ Status", "Success", "Data retrieved successfully")
            if symbol_count:
                exchange_table.add_row("📈 Symbols", str(symbol_count), "Successfully fetched")
            if fetch_time:
                exchange_table.add_row("⏱️ Duration", f"{fetch_time:.1f}s", "Total fetch time")
            ...
            if fetch_time:
                exchange_table.add_row("⏱️ Duration", f"{fetch_time:.1f}s", "Time before failure")
    ...
    def _add_symbol_summary_to_table(self, table: Table, show_failures: bool = False):
        ...
        successful = sum(1 for s in self.symbol_progress.values() if s['status'] == 'success')
        cached = sum(1 for s in self.symbol_progress.values() if s['status'] == 'cached')
        resampled = sum(1 for s in self.symbol_progress.values() if s['status'] == 'resampled')
        failed = sum(1 for s in self.symbol_progress.values() if s['status'] == 'failed')
        ...
        if failed > 0:
            ...
            if show_failures:
                failed_symbols = [sym for sym, data in self.symbol_progress.items() 
                                if data['status'] == 'failed']
                logger.error(f"Failed symbols: {failed_symbols}")
    
    def log_symbol_progress(self, completed: int, total: int, current_symbol: Optional[str] = None):
        """Log progress during symbol fetching."""
        utc_time = self._get_utc_time()
        
        if current_symbol:
            console.print(f"      📈 Progress ({utc_time}): {completed}/{total} symbols | Current: {current_symbol}")
        else:
            console.print(f"      📈 Progress ({utc_time}): {completed}/{total} symbols")
    ...
    def log_final_result(self, data: Optional[vbt.Data], saved: bool = False):
        ...
        was_cache_hit = (
            len(self.operation_path) == 1 and 
            self.operation_path[0][0] == 'cache' and 
            self.operation_path[0][1] == 'hit'
        )
        ...
        if was_cache_hit:
            results_table.add_row("✅ Status", "Cache Hit", "No fetch needed - data already fresh")
        else:
            results_table.add_row("✅ Status", "Success", "Data fetch completed")
            
        results_table.add_row("📊 Symbols", str(symbol_count), ", ".join(data.symbols[:3]) + ("..." if symbol_count > 3 else ""))
        results_table.add_row("⏱️ Total Time", f"{total_time:.1f}s", "End-to-end duration")
        results_table.add_row("🔄 Path", self._get_path_description(), "Data source strategy")
        ...
        try:
            if hasattr(data, 'index') and len(data.index) > 0:
                ...
                results_table.add_row("📊 Candles", str(total_candles), "Total data points")
                
                # Validate data completeness
                validation_result = self._validate_data_completeness(data)
                if validation_result['complete']:
                    results_table.add_row("✅ Completeness", "Good", f"Expected: {validation_result['expected']}, Found: {validation_result['actual']}")
                else:
                    results_table.add_row("⚠️ Completeness", "Issues", f"Missing {validation_result['missing']} candles")
        except Exception as e:
            results_table.add_row("❌ Validation", "Error", f"Could not validate: {str(e)}")
        ...
        if was_cache_hit:
            logger.info(f"[{utc_time}] Cache hit completed in {total_time:.1f}s: {symbol_count} symbols (no fetch needed)")
        else:
            logger.info(f"[{utc_time}] Data fetch completed in {total_time:.1f}s: {symbol_count} symbols via {self._get_path_description()}")
        ...
        if self.symbol_progress:
            ...
            if valid_times:
                ...
                logger.debug(f"[{utc_time}] Average fetch time per symbol: {avg_time:.2f}s")
                logger.debug(f"[{utc_time}] Performance calculated from {len(valid_times)}/{len(self.symbol_progress)} symbols")
    ...
    def _validate_data_completeness(self, data: vbt.Data) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_periods = int((end_time - start_time).total_seconds() / (tf_minutes * 60)) + 1
