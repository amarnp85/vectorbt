...
class DataFetcher:
    ...
    def fetch_data(
        ...
        try:
            ...
            cache_handler = CacheHandler(
                self.exchange_id, timeframe, self.market_type
            )
            
            # Step 3: Try cache
            if use_cache:
                cached_data, cached_symbols, missing_symbols, stale_symbols = cache_handler.try_load_cached(
                ...
                if cached_data and not missing_symbols and not stale_symbols:
                    ...
                    logger.info(f"All requested symbols found in fresh cache - no fetch needed")
                    return result_data
                    ...
                    if updated_stale_data or missing_data:
                        ...
                        merged = cached_data.select_symbols(list(cached_symbols)) if cached_symbols else None
                        ...
                        if updated_stale_data:
                            ...
                            if merged:
                                merged = self.merger.merge_symbol_sets(
                                    merged, cached_symbols,
                                    updated_stale_data, stale_symbols
                                )
                            else:
                        ...
                        if missing_data:
                            ...
                            if merged:
                                merged = self.merger.merge_symbol_sets(
                                    merged, final_symbols - missing_symbols,
    ...
    def _update_stale_symbols(
        ...
        try:
            ...
            for symbol in stale_symbols:
                try:
                    symbol_data = VBTDataHandler.get_symbol_data(cached_data, symbol, 'close')
            ...
            if not latest_timestamps:
                ...
                return self._fetch_missing_symbols(
                    stale_symbols, timeframe, {}, end_date, 
                    cache_handler, prefer_resampling, fetch_logger
                )
            
            # Calculate update start date (next period after latest data)
            # Add one period to avoid duplicate data
            tf_minutes = cache_handler.freshness_checker.parse_timeframe_minutes(timeframe)
            ...
            for symbol in stale_symbols:
                if symbol in latest_timestamps:
                    ...
                    update_start_dates[symbol] = next_period.strftime('%Y-%m-%d %H:%M:%S')
            
            logger.debug(f"Updating {len(stale_symbols)} stale symbols from recent timestamps")
            ...
            if prefer_resampling:
                ...
                if resampled:
                    ...
                    for symbol in stale_symbols:
                        if symbol in latest_timestamps:
                            symbol_data = VBTDataHandler.get_symbol_data(resampled, symbol, 'close')
                            if symbol_data is not None:
                                # Get only data after the cached latest
                                new_data = symbol_data[symbol_data.index > latest_timestamps[symbol]]
                                if len(new_data) > 0:
                                    ...
                                    for feature, df in ohlcv.items():
                                        if df is not None and symbol in df.columns:
                                            feature_data = df[symbol][df.index > latest_timestamps[symbol]]
                                            symbol_df[feature.capitalize()] = feature_data
            ...
            update_data = self.exchange_fetcher.fetch_symbols(
                stale_symbols, timeframe, update_start_dates, end_date, 
                parallel=True, fetch_logger=fetch_logger
            )
    ...
    def fetch_top_symbols(
        ...
        """
        # Get top symbols
        top_symbols = self.symbol_resolver.get_top_symbols(
            quote_currency, limit
        )
    ...
    def update_data(
        ...
        try:
            ...
            if updated_data:
                ...
                if fresh_symbols:
                    merged = self.merger.merge_symbol_sets(
                        existing_data, fresh_symbols,
                        updated_data, stale_symbols
                    )
                    if merged:
