...
Examples:
    ...
    python backtester/scripts/interpolate_data.py --dry-run --exchange binance
"""
...
Examples:
  ...
  %(prog)s --dry-run --exchange binance --market spot
    ...
    parser.add_argument('--exchange', type=str, required=True,
                      help='Exchange to interpolate (required)')
    ...
    parser.add_argument('--max-gap', type=int, default=10000,
                      help='Maximum gap size to interpolate (default: 10000)')
    parser.add_argument('--backup', action='store_true',
                      help='Create backup before interpolation')
    ...
    for file_info in filtered_files:
        ...
        try:
            ...
            if total_missing > args.max_gap:
                print(f"   ⚠️  Gap too large: {total_missing} > {args.max_gap} (skipping)")
                processed_count += 1
                continue
            
            if not args.force and total_missing < 100:
                print(f"   ℹ️  Minor gaps: {total_missing} periods (use --force to interpolate)")
                processed_count += 1
                continue
            ...
            if args.dry_run:
                ...
                if args.backup:
                    backup_path = data_storage.backup_data_file(exchange, timeframe, market)
                    if backup_path:
    ...
    if args.dry_run:
        ...
        if interpolated_count > 0:
            ...
            print(f"   python -m backtester.data.health_check.data_healthcheck --exchange {args.exchange}")
